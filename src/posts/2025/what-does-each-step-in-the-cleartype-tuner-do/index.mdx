...
import { Aside, CaptionedImage } from "@/components"
...
<Aside>
  <h2>June 2026 note</h2>
  <p>
    This is one of the more popular posts on this blog, so I’ve made a few small tweaks for clarity since the post was originally published.
  </p>
</Aside>
}
...
(You can click on those images to see larger versions.)
...
stem of the letter r (indicating that it’s the wrong option for my monitor).
...
and `0` means BGR. (See
[The different registry key locations](#the-different-registry-key-locations)
below for info on what the different registry locations affect.)
...
to `0` (meaning ‘flat’, and causing greyscale anti-aliasing to be used where the
value is supported).
...
This step only appears when tuning the primary display. If you have multiple
displays, this step will be skipped for non-primary displays, and the step
numbering in the titles of the other steps will be adjusted accordingly.

The step sets the `FontSmoothingGamma` registry value under the
...
The second value set is `TextContrastLevel`. The
...
This is a contrast enhancement setting specifically for the greyscale
anti-aliasing mode in DirectWrite (and Direct2D) that applications can opt to
use. (And the DirectWrite documentation
[does not say anything else about the setting](https://learn.microsoft.com/en-gb/windows/win32/api/dwrite_1/nf-dwrite_1-idwriterenderingparams1-getgrayscaleenhancedcontrast).)
...
The tuner also sets the per-monitor `GammaLevel` values under
...
## The different registry key locations

Different applications, and even different parts of the same application (such
as in File Explorer), will be affected by different registry settings. It
depends on the text rendering API the application uses and how the application
uses that API.

The `FontSmoothingOrientation` and `FontSmoothingGamma` values under
`HKEY_CURRENT_USER\Control Panel\Desktop` will normally only affect applications
using GDI to render text (and apply to all displays).

The values under the `Avalon.Graphics` keys will normally be used by
applications using DirectWrite or WPF. Some applications using DirectWrite may
not support the per-display parameters, however, just use the values for the
primary display.
...
There are also limited notifications provided to applications when the ClearType
Text Tuner changes settings, so you may need to close and reopen some apps after
clicking the final ‘Finish’ button to see the effect. (In the case of File
Explorer, you’ll probably need to restart `explorer.exe` using Task Manager.)

...
required running the Disk Clean-up[^1] tool as admin to get rid of the
`Windows.old` directory (created by the in-place upgrade process) and to clean
up a few other things. I also ran the `Dism.exe` clean-up commands (which were
now working!).
...
[^1]:
    As it’s called when Windows is set to use UK English as the display language
    – it’s ‘Disk Cleanup’ (without hyphen) when US English is in use.
